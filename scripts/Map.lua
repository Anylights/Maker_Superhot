...
function Map.Init(scene)
    ...
    if not skipVisuals_ then
        ...
        shadowMat_:SetTechnique(0, alphaTech)
...
function Map.CreateBlockNode(parent, gx, gy, blockType)
    ...
    if not skipVisuals_ then
        ...
        -- 阴影子节点（在方块后面 Z+0.8，向右下偏移，醒目）
        if shadowMat_ then
            local shadowNode = node:CreateChild("Shadow", LOCAL)
            shadowNode.position = Vector3(0.25, -0.25, 0.8)
            shadowNode.scale = Vector3(bs * 1.08, bs * 1.08, 0.02)
            local shadowModel = shadowNode:CreateComponent("StaticModel", LOCAL)
            shadowModel.model = boxModel_
