...
function HUD.DrawQuickMatching()
    ...
    local clientMod = _G.ClientModule
    local playerCount, timeLeft, required = 0, 0, Config.NumPlayers
    if clientMod and clientMod.GetMatchInfo then
        playerCount, timeLeft, required = clientMod.GetMatchInfo()
    end
    local humanCount = playerCount
    ...
    nvgFontSize(vg_, 18)
    nvgTextAlign(vg_, NVG_ALIGN_CENTER + NVG_ALIGN_TOP)
    local secs = math.max(0, math.ceil(timeLeft or 0))
    nvgFillColor(vg_, nvgRGBA(255, 200, 100, 230))
    nvgText(vg_, cx, slotY + slotSize + 36, secs .. "s 后用 AI 补齐开局")
...
--- 已移除：与朋友玩 / 房间相关 UI（DrawFriendMenu / DrawRoomWaiting / DrawRoomJoining）
--- 当前版本仅保留快速匹配功能
