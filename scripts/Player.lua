...
function Player.Init(scene, mapRef)
    ...
    -- 创建阴影材质（半透明深色，与 Map 一致，足够醒目）
    shadowMat_ = Material:new()
    shadowMat_:SetTechnique(0, pbrAlphaTechnique_)
    shadowMat_:SetShaderParameter("MatDiffColor", Variant(Color(0.03, 0.01, 0.06, 0.55)))
    shadowMat_:SetShaderParameter("MatEmissiveColor", Variant(Color(0.0, 0.0, 0.0)))
    shadowMat_:SetShaderParameter("Metallic", Variant(0.0))
    shadowMat_:SetShaderParameter("Roughness", Variant(1.0))
...
function Player.CreateVisuals(node, index)
    ...
    -- 阴影子节点（在玩家后面 Z+0.8，向右下偏移，醒目）
    if shadowMat_ then
        local bs = Config.BlockSize
        local shadowNode = visualNode:CreateChild("Shadow", LOCAL)
        shadowNode.position = Vector3(0.28, -0.28, 0.8)
        shadowNode.scale = Vector3(bs * 1.15, bs * 1.15, 0.02)
        local shadowModel = shadowNode:CreateComponent("StaticModel", LOCAL)
        shadowModel.model = cache:GetResource("Model", "Models/Box.mdl")
        shadowModel.castShadows = false
        shadowModel:SetMaterial(shadowMat_)
    end
