...
local PARAMS = {
    ...
    { key = "JumpSpeed",       label = "跳跃力度",     min = 3,    max = 25,   step = 0.5,  format = "%.1f" },
...
PARAMS[8].apply = function(val)  -- GravityY
    if scene_ then
...
PARAMS[9].apply = function(val)  -- Friction
    TuningPanel.ApplyBodyParam("friction", val)
end

PARAMS[10].apply = function(val)  -- LinearDamping
    TuningPanel.ApplyBodyParam("linearDamping", val)
end

PARAMS[11].apply = function(val)  -- Mass
